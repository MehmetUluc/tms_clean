...
try {
    // Temizlik
    echo "Mevcut veriler temizleniyor...\n";
    ...
    DB::table('hotel_hotel_amenity')->delete();
    DB::table('hotel_hotel_tag')->delete();
    DB::table('hotel_contacts')->delete();
    DB::table('hotels')->delete();
    
    // --------- BÖLGELER ---------
    echo "Bölge verileri kontrol ediliyor...\n";
    ...
    echo "Mevcut bölge sayısı: {$regionCount}\n";
    if ($regionCount < 10) {
        echo "Bölgeler ekleniyor...\n";
    ...
    // --------- OTEL TİPLERİ ---------
    echo "Otel tipleri kontrol ediliyor...\n";
    
    $hotelTypeCount = DB::table('hotel_types')->count();
    if ($hotelTypeCount < 3) {
    ...
    // --------- OTEL ETİKETLERİ ---------
    echo "Otel etiketleri kontrol ediliyor...\n";
    
    $tagCount = DB::table('hotel_tags')->count();
    if ($tagCount < 5) {
    ...
    // --------- PANSİYON TİPLERİ ---------
    echo "Pansiyon tipleri kontrol ediliyor...\n";
    
    $boardTypeCount = DB::table('board_types')->count();
    if ($boardTypeCount < 5) {
    ...
    // --------- ODA TİPLERİ ---------
    echo "Oda tipleri kontrol ediliyor...\n";
    
    $roomTypeCount = DB::table('room_types')->count();
    if ($roomTypeCount < 4) {
    ...
    // --------- ODAL ÖZELLİKLERİ ---------
    echo "Oda özellikleri kontrol ediliyor...\n";
    
    $roomAmenityCount = DB::table('room_amenities')->count();
    if ($roomAmenityCount < 6) {
    ...
    // --------- OTEL ÖZELLİKLERİ ---------
    echo "Otel özellikleri kontrol ediliyor...\n";
    
    $hotelAmenityCount = DB::table('hotel_amenities')->count();
    if ($hotelAmenityCount < 6) {
    ...
    foreach ($hotels as $hotel) {
        ...
        if (!$hotelExists) {
            ...
            // Var olan oteli bul
            $hotelId = DB::table('hotels')->where('slug', Str::slug($hotel['name']))->value('id');
            $hotelIds[] = $hotelId;
        }
    }
    
    // Hotel ID'lerini yazdır
    echo "Otel ID'leri: " . implode(", ", $hotelIds) . "\n";
    ...
    if ($roomTypes->isEmpty()) {
        throw new Exception("Oda tipleri bulunamadı. Lütfen önce oda tiplerini ekleyin.");
    }
    
    // Oda özellik ID'lerini al
    $amenityIds = DB::table('room_amenities')->pluck('id')->toArray();
    if (empty($amenityIds)) {
        throw new Exception("Oda özellikleri bulunamadı. Lütfen önce oda özelliklerini ekleyin.");
    }
    
    // Pansiyon tiplerini al
    $boardTypes = DB::table('board_types')->get();
    if ($boardTypes->isEmpty()) {
        throw new Exception("Pansiyon tipleri bulunamadı. Lütfen önce pansiyon tiplerini ekleyin.");
    }
    
    // Her otel için odalar oluştur
    foreach ($hotelIds as $hotelId) {
        $hotelName = DB::table('hotels')->where('id', $hotelId)->value('name');
        echo "  {$hotelName} için odalar oluşturuluyor...\n";
        
        foreach ($roomTypes as $roomType) {
            echo "    {$roomType->name} tipi odalar oluşturuluyor...\n";
            ...
            for ($i = 1; $i <= 2; $i++) {
                ...
                echo "      Oda: {$roomType->name} {$i} (ID: {$roomId}) oluşturuldu\n";
