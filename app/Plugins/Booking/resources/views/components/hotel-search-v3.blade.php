                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center text-sm">
                        <div class="price-tag-simple">
                            ...
                            <span class="text-lg font-bold text-gray-900 dark:text-gray-100">₺{{ number_format($filters['priceRange'][0]) }}</span>
                        </div>
                        ...
                        <div class="price-tag-simple text-right">
                            ...
                            <span class="text-lg font-bold text-gray-900 dark:text-gray-100">₺{{ number_format($filters['priceRange'][1]) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs text-gray-500">
                            ...
                            <span wire:loading.class="opacity-50" wire:target="priceRange">₺{{ number_format($filters['priceRange'][0]) }}</span>
                        </div>
                        <input type="range" 
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs text-gray-500">
                            ...
                            <span wire:loading.class="opacity-50" wire:target="priceRange">₺{{ number_format($filters['priceRange'][1]) }}</span>
                        </div>
                        <input type="range" 
        ...
        @if(count($selectedRooms) > 0)
            <div class="selected-rooms-summary">
                ...
                <div class="text-xl font-bold mt-1">
                    Total: ₺{{ number_format(array_sum(array_column($selectedRooms, 'total_price'))) }}
                </div>
            </div>
        @endif
    </div>
                                ...
                                <div class="text-right min-w-[150px]">
                                    <div class="mb-4">
                                        @if($hotel->original_price ?? null)
                                            <p class="text-sm text-gray-500 line-through">₺{{ number_format($hotel->original_price) }}</p>
                                        @endif
                                                    ...
                                                    <div class="flex flex-col">
                                                        ...
                                                        <div class="flex items-start justify-between mb-3">
                                                            <div class="flex items-center gap-3 flex-1">
                                                                @if($boardType->icon)
                                                                    @php
                                                                        ...
                                                                        $displayIcon = $iconMap[$boardType->icon] ?? '📋';
                                                                    @endphp
                                                            ...
                                                            <div class="flex items-start gap-4 ml-4">
                                                                    ...
                                                                    @if($averagePrice > 0)
                                                                        ...
                                                                        <div class="space-y-1 border-t border-gray-200 dark:border-gray-700 pt-2">
                                                                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                                                                ₺{{ number_format($averagePrice) }} {{ $ratePlan->is_per_person ? 'per person' : 'per room' }}/night
                                                                            </p>
                                                                ...
                                                                @endphp
                                                                <button type="button"
                                                                        wire:click="{{ $ratePlanIsSOR ? 'handleSaleOnRequest(' . json_encode($sorData) . ')' : ($isSelected ? 'removeRoom(' . $selectedIndex . ')' : 'addRoom(' . json_encode($roomData) . ')') }}"
                                                                        class="rate-plan-button {{ $ratePlanIsSOR ? 'sor-button' : ($isSelected ? 'selected' : '') }} {{ (!$isAvailable || $averagePrice <= 0) ? 'opacity-50 cursor-not-allowed' : '' }}"
                                                        ...
                                                        <div>
                                                            ...
                                                            <div class="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                                                                <span class="flex items-center gap-1">
                                                                    {{ $ratePlan->is_per_person ? '👤 Per person' : '🏠 Per room' }}
                                                                </span>
                                                                @if($dailyRates->first())
                                                                    <span class="flex items-center gap-1">
        ...
        <div class="sticky bottom-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4 -mx-4">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-lg font-semibold">Total: ₺{{ number_format(array_sum(array_column($selectedRooms, 'total_price'))) }}</p>
                </div>
                <button type="button"
