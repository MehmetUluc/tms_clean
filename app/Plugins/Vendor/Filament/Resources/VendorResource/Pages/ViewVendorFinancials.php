...
class ViewVendorFinancials extends Page
{
    protected static string $resource = VendorResource::class;

    protected static ?string $title = 'Financial Summary';

    protected static string $view = 'filament.pages.view-vendor-financials';
                        ...
                        Card::make()
                            ->schema([
                                ...
                                TextEntry::make('balance')
                                    ...
                                    ->formatStateUsing(fn () => number_format($this->summary['balance'] ?? 0, 2) . ' ' . config('vendor.default_currency'))
                                    ->size('xl')
                                    ->weight('bold')
                        ...
                        Card::make()
                            ->schema([
                                ...
                                TextEntry::make('total_amount')
                                    ->label('Total Amount')
                                    ->formatStateUsing(fn () => number_format($this->summary['transactions']['total_amount'] ?? 0, 2) . ' ' . config('vendor.default_currency'))
                                    ->size('xl')
                                    ->weight('bold'),
                                
                                TextEntry::make('total_commission')
                                    ->label('Total Commission')
                                    ->formatStateUsing(fn () => number_format($this->summary['transactions']['total_commission'] ?? 0, 2) . ' ' . config('vendor.default_currency'))
                                    ->color('danger'),
                                
                                TextEntry::make('total_net_amount')
                                    ->label('Total Net Amount')
                                    ->formatStateUsing(fn () => number_format($this->summary['transactions']['total_net_amount'] ?? 0, 2) . ' ' . config('vendor.default_currency'))
                                    ->color('success'),
                            ]),
                        ...
                        Card::make()
                            ->schema([
                                ...
                                TextEntry::make('total_payments')
                                    ...
                                    ->formatStateUsing(fn () => number_format($this->summary['payments']['total_payments'] ?? 0, 2) . ' ' . config('vendor.default_currency'))
                                    ->size('xl')
                                    ->weight('bold')
                                    ->color('success'),
                                ...
                                TextEntry::make('pending_payment_requests')
                                    ...
                                    ->formatStateUsing(fn () => number_format($this->summary['payments']['pending_payment_requests'] ?? 0) . ' (' . number_format($this->summary['payments']['pending_payment_amount'] ?? 0, 2) . ' ' . config('vendor.default_currency') . ')'),
                            ]),
                    ]),
            ]);
    }
        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('approve')
                    ...
                    ->color('success')
                    ->visible(fn (VendorPaymentRequest $record): bool => $record->status === 'pending')
                    ->form([
                        Forms\Components\Textarea::make('notes')
                            ->label('Notes')
                            ->maxLength(500),
                    ])
                    ->action(function (VendorPaymentRequest $record, array $data): void {
                        $vendorService = app(VendorService::class);
                        $vendorService->processPayment(
                            $record,
                            'approved',
                            null,
                            null,
                ...
                Tables\Actions\Action::make('markAsPaid')
                    ...
                    ->form([
                        ...
                        Forms\Components\Select::make('payment_method')
                            ...
                            ->options(config('vendor.payment_methods'))
                            ->required()
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('payment_method')
                    ->options(array_map(
                        ...
                        array_keys(config('vendor.payment_methods'))
                    )),
            ])
            ->actions([
                Tables\Actions\ViewAction::make(),
            ...
            Actions\Action::make('createPaymentRequest')
                ...
                ->form([
                    ...
                    Forms\Components\Select::make('bank_account_id')
                        ->label('Bank Account')
                        ->options(function () {
                            return $this->vendor->bankAccounts()
                                ->get()
                ...
                ->action(function (array $data): void {
                    $vendorService = app(VendorService::class);
                    $vendorService->createPaymentRequest(
                        $this->vendor,
                        $data['amount'],
