...
class VendorDocuments extends Page
{
    protected static ?string $navigationIcon = 'heroicon-o-document-text';

    protected static string $view = 'filament.pages.vendor-documents';
    ...
    {
        $documentTypes = config('vendor.document_types', []);
        
        return $form
            ->schema([
                Forms\Components\Section::make('Upload Document')
                    ->description('Upload a new document for your vendor account')
                    ->schema([
        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\IconColumn::make('is_expired')
                    ->label('Expired')
                    ->boolean()
                    ->getStateUsing(fn (VendorDocument $record): bool => $record->isExpired())
            ...
            ->filters([
                Tables\Filters\SelectFilter::make('document_type')
                    ->options(config('vendor.document_types', [])),
            ...
            ->actions([
                Tables\Actions\Action::make('download')
                    ...
                    ->color('primary')
                    ->url(fn (VendorDocument $record): string => Storage::url($record->file_path))
                    ->openUrlInNewTab(),
                ...
                Tables\Actions\DeleteAction::make()
                    ->visible(fn (VendorDocument $record): bool => $record->status === 'pending'),
            ])
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make()
                        ->action(function (Collection $records): void {
                            ...
                            $records = $records->filter(fn (VendorDocument $record) => $record->status === 'pending');
